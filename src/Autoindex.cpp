...
std::string urlEncode(const std::string &input) {
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    // Caracteres seguros (solo estos carácteres pueden ir sin codificar,
    // siguiendo el estándar RFC)
...
Como esta función solo se usa para autoindex / paths, el espacio siempre es %20,
no hay que distinguir con query ni usar el +
...
void handleDirectory(Client *client, const std::string &dirPath,
  ...
  if (!defaultFile.empty() && stat(indexPath.c_str(), &st) == 0 &&
      S_ISREG(st.st_mode)) {
    std::cout << "[DEBUG] Encontrado index: " << indexPath << std::endl;
    client->serveStaticFile(
        indexPath); // Delegamos la lectura y envío al método de Client
    return;
  }
  ...
  if (autoindexEnabled) {
    std::cout << "[DEBUG] Autoindex ON → Generando listado HTML para: "
              << dirPath << std::endl;
    generateListing(client, dirPath, urlPath, location);
    return;
  }
...
Cuando haces:
    ...
    {
        return Autoindex::handleDirectory(this, realPath, requestUrlPath,
loc.autoindex, loc.defaultFile);
    }
...
Parámetros:
    Client* client — puntero al objeto Client que representa la conexión actual;
usado para delegar acciones como servir archivos o enviar respuestas.
...
enlaces.
    ...
    autoindexEnabled — flag (de tu configuración) que decide si mostramos
listado.

    defaultFile — nombre del archivo por defecto (p. ej. "index.html").

Valor devuelto: true si la operación dejó preparado _httpResponse correcto y el
flujo sigue; false si hubo error (el Client esperará limpiar/close según
lógica).

¿Qué hace conceptualmente la función?
handleDirectory() recibe:
    client → para poder enviar respuestas y porque necesitamos usar
client->serveStaticFile() y client->sendError() dirPath → ruta real del
directorio en disco urlPath → ruta tal como la pidió el navegador (p.ej.
/fotos/) autoindexEnabled → si está activado el autoindex en esta location
...
2. Reserva una estructura stat para obtener info del archivo
    ...
    Si ambas son ciertas → tenemos index.html, así que delega en:
        return client->serveStaticFile(indexPath);
            Qué se espera de serveStaticFile: que prepare _httpResponse con
headers+body del archivo y devuelva true si todo ok. Lo servimos y terminamos ->
return permite que el flujo normal del Client continúe
...
void generateListing(Client *client, const std::string &dirPath,
  ...
  if (!dir) {
    ...
    if (errno == EACCES)
      client->sendError(403, &location); // Forbidden
    else
      client->sendError(404, &location); // Not Found
    return;
  }
  ...
  return; // TODO: REVISAR SI SE USA EL RETURN DE ESTA FUNCION EN OTROS LADOS.
          // AQUI NO SE USA
}
...
directorio. Es lo que en Apache o Nginx se llama autoindex.
    ...
    Y no hay index.html pero la config tiene autoindex on;, tu server devuelve
una página con: nombre de cada archivo/subdirectorio fecha de modificación
        tamaño
        link clicable
...
if (!dir)
{
    ...
    opendir() intenta abrir el directorio real del sistema (ej.
/var/www/html/fotos).

    Si falla → no podemos leer la carpeta.
    En vez de 404 se devuelve 403 Forbidden, porque técnicamente sí existe, pero
no se puede acceder.
...
3. Cabecera HTML + CSS
    ...
    TODO: recomendación de seguridad: escapar HTML si existe posibilidad de
caracteres especiales — si urlPath proviene de la request debería escaparse para
evitar XSS en nombres raros. (Más abajo doy la variante con escapeHtml.)
...
4. Título y apertura de tabla
    ...
    html << "<table><tr><th>Name</th><th>Last
...
5. Iterar por los archivos del directorio
    ...
    🔸 Saltar . y ..
        ...
        TODO: REVISAR: (Algunas implementaciones muestran .. como enlace al
padre; puedes añadirlo explícitamente si quieres.)
...
Lo que debe aparecer en el listado del directorio es esto (que es como se ve el
listado de Nginx/Apache):
    ...
    ...
            Eso te da:
                ...
                🔹 Saber si es directorio añadiendo una barra /
                    ...
                    La barra final es un estándar visual y funcional (facilita
rutas relativas).

                🔹 Estilo CSS
                    Para que la tabla se vea bien (alineada, legible,
consistente).
...
Por lo tanto, seguiremos con lo siguiente:
    ...
    🔸 Formatear la fecha de modificación en formato legible
        ...
        Convierte el timestamp en formato YYYY-MM-DD HH:MM:SS.
            TODO: REVISAR: localtime no es reentrant (usa estado interno); para
tu proyecto (single-threaded webserv) está bien. Si multithread, usar
localtime_r
...
6.Insertar fila en la tabla
    ...
    TODO: IMPORTANTE: aquí no estamos haciendo URL-encoding ni HTML-escaping —
en la implementación final deberías usar urlEncode(name) para la URL y
escapeHtml(displayName) para el texto mostrado; lo explico abajo en la sección
de seguridad.

    TODO: urlPath.back() se usa para comprobar el último carácter; asegúrate de
que urlPath no esté vacío antes de llamar back().
...
std::to_string(htmlStr.size())); _httpResponse.setBody(htmlStr);
