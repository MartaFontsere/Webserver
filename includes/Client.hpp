...
public:
  ...
  bool sendError(int errorCode, const LocationConfig *location = NULL);

private:
  int _clientFd; // file descriptor del socket del cliente
  sockaddr_in
      _addr; // dirección IP y puerto del cliente   // TODO *****SI DEJO ESTE
             // PONER IGUAL EN EL
             // SERVER!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  std::string _rawRequest; // buffer con los datos RAW recibidos del socket
                           // Client es responsable del socket y de recibir
                           // bytes. → Por tanto, debe guardar temporalmente lo
                           // que va llegando desde el socket

  // bool _keepAlive;      // para saber si la conexión se debe mantener viva
  // después de mandar una petición o no
  HttpRequest _httpRequest;
  // HttpRequest no sabe nada del socket. Solo sabe parsear texto una vez lo
  // tiene completo. → Su función es transformar texto crudo → estructura
  // interpretada (método, headers, body...).
  ...
  size_t _writeOffset; // bytes ya enviados desde el inicio de _writeBuffer.
                       // indica cuánto ya has enviado — así no reenvías bytes
                       // ya enviados.
  time_t _lastActivity; // timestamp del último recv/send exitoso. te permitirá
                        // implementar timeouts (más tarde)
  bool _requestComplete;
  std::vector<ServerConfig> _servCandidateConfigs;
  const ServerConfig *_matchedConfig;

  HttpResponse _httpResponse;

  // Límite razonable para servir archivos en memoria. Ajustar según recursos.
  static const size_t MAX_STATIC_FILE_SIZE =
      10 * 1024 * 1024; // TODO: 10 MB SE PUEDE DECLARAR ASI A PELO?
...
*/
  ...
  bool validateMethod();
  bool handleGet(const LocationConfig &location);
  bool handleHead(const LocationConfig &location);
  bool handlePost(const LocationConfig &location);
  bool handleDelete(const LocationConfig &location);
  std::string urlDecode(const std::string &encoded, bool plusAsSpace) const;
  std::string getDecodedPath(const std::string &rawPath) const;
  std::string getDecodedQuery(const std::string &rawQuery) const;
  std::string sanitizePath(const std::string &path);
  std::string buildFullPath(const std::string &cleanPath);
  bool readFileToString(const std::string &fullPath, std::string &out,
                        size_t size); // Helpper para serveStaticFile

  std::string determineMimeType(const std::string &path);

  // HARDCODEADO, va en configfile -> Funciona como lookup rápido para saber qué
  // Content-Type poner según extensión
...
Por qué la necesitamos
    ...
    Por eso conviene crear una clase Client que represente a cada cliente
conectado.


    Explicación:

        _clientFd → es el identificador del socket que representa a este
cliente.

        _addr → contiene la dirección IP y el puerto del cliente (lo llena
accept()).

        _buffer → servirá para almacenar lo que el cliente envía (por si llega
por trozos).

        _closed → nos permite marcar si el cliente ya cerró la conexión, y así
poder eliminarlo del poll() más tarde.
*/
...
¿Dónde declarar HttpRequest _httpRequest?
    ...
    Por qué:
        Cada Client representa una conexión individual, por tanto, su
HttpRequest también es única.

        Nadie fuera de Client debería modificar los datos crudos del request,
solo leerlos.

        Desde fuera (por ejemplo, en Server), accederás a los datos a través de
getters o referencias controladas, no modificando directamente _httpRequest.
*/